...
function BookInput() {
  ...
  // 매개변수를 따로 사용하지 않고 useBookContext만 사용하면 state 들을
  // 다 받아올 수 있다.

  /**
   * react에서 일반적으로 선언한 변수는 let nextId = 0;
   * 실제 화면전체가 변경되지 않더라도
   *      화면전체가 변경 : refresh
   *      react는 화면을 refresh하지 않ㄷ고 rendering한다.
   *      데이터(state)가 변경 되었을때만 reRendering된다.
   * 변수는 무조건 초기화 되어 버린다.
   ...
   *
   * 그럴때 reference 변수로 선언을 해주어야 한다.
   */
  // let nextId = 0;
  const nextId = useRef(0); // 이코드를 사용하면 강제로 f5를 누르지 않는이상 값이 변하지 않는다.
  /**
   * nextId라는 변수를 만들고
  ...
  const bookInsert = async () => {
    ...
    // nextId++;
    // Ref로 선언된 변수의 current 요소를 1 증가하며
    // 다음번 list의 id로 사용한다.
    nextId.current++;
