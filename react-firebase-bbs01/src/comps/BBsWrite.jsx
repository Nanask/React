...
function BBsWrite({ history }) {
  ...
  // onChange Event 핸들러
  // 키보드로 입력한 데이터를 bbs 객체에 setting하는 일을 수행한다.
  const onChange = (e) => {
  ...
  const onClickSave = () => {
    ...
    firestore
      .collection("bbs")
      .add(saveBBS)
      // .doc("Hello") // 이문서의 데이터를 업데이트 시켜라
      // .set(saveBBS)
      .then((result) => {
        // console.log(result);
        history.push("/"); // 입력이 완료가 되면 홈으로 가라!
      });
  };
  ...
  return (
    <div>
      <div>
        ...
        */}
        <input name="b_write" type="text" onChange={onChange} placeholder="작성자" />
      </div>
      <div>
        <input name="b_subject" type="text" onChange={onChange} placeholder="제목" />
      </div>
      <div>
        <input name="b_content" type="text" onChange={onChange} placeholder="내용" />
      </div>
      <div>
